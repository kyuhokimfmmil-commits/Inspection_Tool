...
if start_review:
    if not api_key:
        ...
        try:
            ...
            - 오류: "옳지 않은 것"을 찾으라고 했고 정답표에 3번이라고 되어있는데, 해설을 읽어보니 "3번 선지는 맞는 지문(O)이다"라고 설명하고 있는 경우. (앞뒤가 안 맞음)
            - 정답 번호 불일치: 표와 제목에는 정답이 4번이라고 적혀있는데, 상세 해설 텍스트에서는 3번이 정답이라고 딴소리를 하는 경우.
            - 개수형 문제의 경우 문제와 해설간의 연결성에 오류가 없는지 검토
            ...
            [출력 지시사항]
            오직 아래 두 가지만 출력해.
            정답표 누락: 1번부터 40번 중 정답표에 빈칸으로 누락된 번호만 나열할 것.
